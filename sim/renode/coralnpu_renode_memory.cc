...
CoralNPURenodeMemory::CoralNPURenodeMemory(uint64_t block_size_bytes,
  ...
  // Available memory should be greater than CoralNPU's default 4MB address
  // space.
  ABSL_HARDENING_ASSERT(max_address_ >
...
bool CoralNPURenodeMemory::IsValidAddress(uint64_t address,
                                          uint64_t high_address) {
  return (address >= base_address_) && (high_address <= max_address_);
}

void CoralNPURenodeMemory::Load(uint64_t address, DataBuffer* db,
...
void CoralNPURenodeMemory::Load(DataBuffer* address_db, DataBuffer* mask_db,
...
void CoralNPURenodeMemory::Store(uint64_t address, DataBuffer* db) {
  int size_in_units = db->size<uint8_t>() / addressable_unit_size_;
  uint64_t high = address + size_in_units;
  ABSL_HARDENING_ASSERT(IsValidAddress(address, high));
  ABSL_HARDENING_ASSERT(size_in_units > 0);
...
void CoralNPURenodeMemory::Store(DataBuffer* address_db, DataBuffer* mask_db,
...
void CoralNPURenodeMemory::LoadStoreHelper(uint64_t address, uint8_t* byte_ptr,
