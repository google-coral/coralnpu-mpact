...
#include "sim/coralnpu_bin_decoder.h"
...
inline void GetVRegGroup(
    CoralNPUState* state, int reg_num, bool strip_mine, int widen_factor,
...
template <typename RegType>
inline SourceOperandInterface* GetVectorRegisterSourceOp(CoralNPUState* state,
                                                         int reg_num,
                                                         bool strip_mine,
                                                         int widen_factor) {
  std::vector<mpact::sim::generic::RegisterBase*> vreg_group;
...
inline DestinationOperandInterface* GetVectorRegisterDestinationOp(
    CoralNPUState* state, int reg_num, bool strip_mine, bool widening,
    int latency) {
  std::vector<mpact::sim::generic::RegisterBase*> vreg_group;
...
inline DestinationOperandInterface* GetRegisterDestinationOp(
    CoralNPUState* state, std::string name, int latency) {
...
template <typename RegType>
inline DestinationOperandInterface* GetRegisterDestinationOp(
    CoralNPUState* state, std::string name, int latency, std::string op_name) {
  auto* reg = state->GetRegister<RegType>(name).first;
...
template <typename RegType>
inline SourceOperandInterface* GetRegisterSourceOp(CoralNPUState* state,
                                                   std::string name) {
  auto* reg = state->GetRegister<RegType>(name).first;
...
CoralNPUEncoding::CoralNPUEncoding(CoralNPUState* state) : state_(state) {
  InitializeSourceOperandGetters();
  InitializeDestinationOperandGetters();
  resource_pool_ = new mpact::sim::generic::SimpleResourcePool("Kelvin", 128);
}

CoralNPUEncoding::~CoralNPUEncoding() { delete resource_pool_; }

void CoralNPUEncoding::InitializeSourceOperandGetters() {
  ...
  source_op_getters_.emplace(
      ...
      [this]() -> SourceOperandInterface* {
        auto reg_num =
            encoding::coral_n_p_u_v2_args_type::ExtractVs1(inst_word_);
        bool strip_mine =
            encoding::coral_n_p_u_v2_args_type::ExtractM(inst_word_);
        auto form = encoding::coral_n_p_u_v2_args_type::ExtractForm(inst_word_);
  ...
  source_op_getters_.emplace(
      ...
      [this]() -> SourceOperandInterface* {
        auto reg_num =
            encoding::coral_n_p_u_v2_args_type::ExtractVs2(inst_word_);
        bool strip_mine =
            encoding::coral_n_p_u_v2_args_type::ExtractM(inst_word_);
        auto form = encoding::coral_n_p_u_v2_args_type::ExtractForm(inst_word_);
  ...
  source_op_getters_.emplace(
      ...
      static_cast<int>(SourceOpEnum::kVd), [this]() -> SourceOperandInterface* {
        auto reg_num =
            encoding::coral_n_p_u_v2_args_type::ExtractVd(inst_word_);
        bool strip_mine =
            encoding::coral_n_p_u_v2_args_type::ExtractM(inst_word_);
        return GetVectorRegisterSourceOp<mpact::sim::riscv::RVVectorRegister>(
            state_, reg_num, strip_mine, 1 /* widen_factor */);
  ...
  source_op_getters_.emplace(
      ...
      [this]() -> SourceOperandInterface* {
        auto reg_num =
            encoding::coral_n_p_u_v3_args_type::ExtractVs3(inst_word_);
...
void CoralNPUEncoding::InitializeDestinationOperandGetters() {
  ...
  dest_op_getters_.insert(std::make_pair(
      ...
      [this](int latency) -> DestinationOperandInterface* {
        ...
        if (num == 0) {
          ...
          return GetRegisterDestinationOp<mpact::sim::riscv::RV32Register>(
              state_, absl::StrCat(CoralNPUState::kXregPrefix, num), latency,
              mpact::sim::riscv::kXRegisterAliases[num]);
        }
      }));
  ...
  dest_op_getters_.insert(std::make_pair(
      ...
      [this](int latency) -> DestinationOperandInterface* {
        ...
        if (reg_num == 0) {
          ...
          reg_num >>= 1;
          return GetRegisterDestinationOp<mpact::sim::riscv::RV32Register>(
              state_, absl::StrCat(CoralNPUState::kXregPrefix, reg_num),
              latency, mpact::sim::riscv::kXRegisterAliases[reg_num]);
        }
      }));
...
void CoralNPUEncoding::ParseInstruction(uint32_t inst_word) {
  inst_word_ = inst_word;
  std::vector<absl::AnyInvocable<isa32::OpcodeEnum(uint32_t inst_word)>>
      decode_functions;
  decode_functions.push_back(encoding::DecodeCoralNPUInst);
  decode_functions.push_back(encoding::DecodeCoralNPUVectorArithInst);
  decode_functions.push_back(encoding::DecodeCoralNPUVectorConvInst);
  decode_functions.push_back(encoding::DecodeCoralNPUVectorMemoryInst);
  decode_functions.push_back(encoding::DecodeCoralNPUVectorMulInst);
  decode_functions.push_back(encoding::DecodeCoralNPUVectorShiftInst);
...
DestinationOperandInterface* CoralNPUEncoding::GetDestination(
    SlotEnum, int, OpcodeEnum opcode, DestOpEnum dest_op, int, int latency) {
...
SourceOperandInterface* CoralNPUEncoding::GetSource(SlotEnum, int,
                                                    OpcodeEnum opcode,
...
bool CoralNPUEncoding::IsWidenDestinationRegisterOp() const {
  auto func1 = encoding::coral_n_p_u_v2_args_type::ExtractFunc1(inst_word_);
  auto func2 = encoding::coral_n_p_u_v2_args_type::ExtractFunc2(inst_word_);
...
int CoralNPUEncoding::GetSrc1WidenFactor() const {
  auto func1 = encoding::coral_n_p_u_v2_args_type::ExtractFunc1(inst_word_);
  auto func2 = encoding::coral_n_p_u_v2_args_type::ExtractFunc2(inst_word_);
  auto sz = encoding::coral_n_p_u_v2_args_type::ExtractSz(inst_word_);
  auto func2_ignore_unsigned = func2 & (~(1u << 0));
