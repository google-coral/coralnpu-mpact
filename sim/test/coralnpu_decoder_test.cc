...
class CoralNPUDecoderTest : public testing::Test {
 protected:
  CoralNPUDecoderTest()
      : memory_(0),
        state_("coralnpu_decoder_test", RiscVXlen::RV32, &memory_),
  ...
  ~CoralNPUDecoderTest() override { delete symbol_accessor_; }

  ELFIO::elfio elf_reader_;
  mpact::sim::util::FlatDemandMemory memory_;
  coralnpu::sim::CoralNPUState state_;
  mpact::sim::util::ElfProgramLoader loader_;
  coralnpu::sim::CoralNPUDecoder decoder_;
...
TEST_F(CoralNPUDecoderTest, HelloWorldMain) {
...
TEST_F(CoralNPUDecoderTest, BadAddress) {
