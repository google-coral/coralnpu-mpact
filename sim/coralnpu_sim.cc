...
static coralnpu::sim::CoralNPUTop* top = nullptr;
...
static bool PrintVectorRegisters(
  ...
  std::string output_str;
  for (int i = 0; i < coralnpu::sim::kNumVregs; ++i) {
    ...
    std::string data_str;
    std::string sep;
    for (int j = 0; j < coralnpu::sim::kVectorLengthInBits / 32; ++j) {
...
int main(int argc, char** argv) {
  absl::InitializeLog();
  absl::SetProgramUsageMessage("CoralNPU MPACT-Sim based CLI tool");
  ...
  coralnpu::sim::CoralNPUTop coralnpu_top("CoralNPU");
  ...
  mpact::sim::util::ElfProgramLoader elf_loader(coralnpu_top.memory());
  ...
  if (is_elf_file) {
    ...
    auto res = coralnpu_top.LoadImage(file_name,
                                      absl::GetFlag(FLAGS_bin_memory_offset));
    if (!res.ok()) {
      LOG(ERROR) << "Error while loading '" << file_name
                 << "': " << res.message();
      return -1;
  ...
  auto pc_write = coralnpu_top.WriteRegister("pc", entry_point);
  if (!pc_write.ok()) {
  ...
  if (interactive) {
    ...
    cmd_shell.AddCore({&coralnpu_top, [&elf_loader]() { return &elf_loader; }});
    ...
    auto run_status = coralnpu_top.Run();
    if (!run_status.ok()) {
      LOG(ERROR) << run_status.message();
    }

    auto wait_status = coralnpu_top.Wait();
    if (!wait_status.ok()) {
      LOG(ERROR) << wait_status.message();
    }
    auto sec = absl::ToDoubleSeconds(absl::Now() - t0);
    std::cout << "Total cycles: " << coralnpu_top.GetCycleCount() << '\n';
